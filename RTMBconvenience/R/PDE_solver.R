...
makeAdvDiff <- function(x0,f,g){
    ...
    J <- function(X,Time){
        matrix(J_tp(c(Time,X)),ncol=length(X)+length(Time))[,-1]
    }
    mu <- function(X,Time){
        vJ <- array(J(X,Time),dim = c(length(X),length(X),length(X)))
...
#' @export
buildSystem <- function(mu,D,rho,S, grd, phi0){
    ## Check input
    ...
    buildEquations <- function(t0,t1, phi0){
        ...
        Sstar <- simplify(lapply(seq_len(numCells), function(i)  S_atomic(c(phi0[i],i)) ))
        dS <- simplify(lapply(seq_len(numCells), function(i) S_atomic$jacobian(c(phi0[i],i))[1,1]))
        S0 <- Sstar - dS * phi0
        S1 <- dS
    ...
    if(RTMB:::ad_context()){
        sparse_solver_ptr <- RTMBconvenience:::sparse_solve_ptr_ad(Eq0$A)
        sparse_solver_update <- function(A) RTMBconvenience:::sparse_solve_ptr_update_ad(sparse_solver_ptr,A)
        sparse_solver_eval <- function(ph) as.vector(RTMBconvenience:::sparse_solve_ptr_eval_ad(sparse_solver_ptr, RTMB::matrix(ph,ncol=1)))
    }else{
